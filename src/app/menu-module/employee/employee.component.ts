...
import { city, country, departments, employee, state } from 'src/app/modal/user';
...
export class EmployeeComponent implements OnInit {
  ...
  constructor(public fb:FormBuilder,private commonService:CommonService,private employeeService:EmployeeService) { }
 ...
 */
  checkError(fieldName:string,errorName:string):boolean | undefined{
    return this.employeeForm.get(fieldName)!.hasError(errorName)&& this.employeeForm.get(fieldName)!.touched;
  }
 ...
 * @returns {boolean | undefined} 
 */
  addressErrorCheck(type:string,fieldName:string,errorName:string):boolean|undefined{
    if(type=='CURRENT'){
      return this.currentAddress.get(fieldName)!.hasError(errorName)&& this.currentAddress.get(fieldName)!.touched;
    }else{
      return this.permanentAddress.get(fieldName)!.hasError(errorName)&& this.permanentAddress.get(fieldName)!.touched;
    }
  }
 ...
 */
  employeeFormSubmit(){
    ...
    let data={
      ...
      addresses:[{...formData.currentAddress},{...formData.permanentAddress}]
    } as employee;
