...
describe('EmployeeComponent', () => {
  ...
  beforeEach(async () => {
    commonServiceMock = jasmine.createSpyObj('CommonService', ['getCountry', 'getAllDepartments', 'getState', 'getCity']);
    const employeeServiceSpy = jasmine.createSpyObj('EmployeeService', ['createEmployee']);
  ...
  it('should load countries and departments on init', () => {
    const mockCountries:country[] = [{id:1, name: 'Country1' }, {id:2, name: 'Country2' }];
    const mockDepartments = { data: [{ id:1,name: 'Dept1' }, {id:2, name: 'Dept2' }] };
  ...
  it('should validate mobile number', () => {
    ...
    expect(component.mobileValidator(invalidControl)).toEqual({ invalidMobile: true });
  });
  ...
  it('should load states when a country is selected', () => {
    const mockStates:state[] = [{ name: 'State1',countryName:'India' }, { name: 'State2',countryName:'India' }];
  ...
  it('should return true if the current address field has the specified error and has been touched', () => {
    ...
    const hasError = component.addressErrorCheck('CURRENT', 'line1', 'required');
  ...
  it('should return true if the permanent address field has the specified error and has been touched', () => {
    ...
    const hasError = component.addressErrorCheck('PERMANENT', 'zip', 'required');
  ...
  it('should return false if the permanent address field does not have the specified error', () => {
    ...
    const hasError = component.addressErrorCheck('PERMANENT', 'state', 'required');
...
it('should reset and load cities for the permanent address when a state is selected', () => {
  const mockCities:city[] = [{ name: 'City1',stateName:'Delhi' }, { name: 'City2',stateName:'Delhi' }];
...
it('should not load cities if the state is empty for current address', () => {
  ...
  it('should fill the permanent address with values from the current address', () => {
    ...
    component.currentAddressCityList = [{ name: 'City1',stateName:'Delhi' }, { name: 'City2',stateName:'Delhi' }];
    component.currentAddressStateList = [{ name: 'State1',countryName:'India' }, { name: 'State2',countryName:'India' }];
    ...
    expect(component.permanentAddressCityList).toEqual(component.currentAddressCityList);
    expect(component.permanentAddressStateList).toEqual(component.currentAddressStateList);
  });
