...
describe('JwtInterceptor', () => {
  ...
  it('should handle 403 error and navigate to login', () => {
    ...
    const httpRequest = new HttpRequest('GET', '/test');
    const httpHandler = {
      handle: () => throwError({ status: 403, error: { error: 'Forbidden' } }),
    };
  ...
  it('should handle 500 error', () => {
    ...
    interceptor.intercept(httpRequest, httpHandler as HttpHandler).subscribe({
      error: (err) => {
        ...
        expect(loggerServiceSpy.errorAlert).toHaveBeenCalledWith(
  ...
  it('should handle other errors and log them', () => {
    ...
    const httpRequest = new HttpRequest('GET', '/test');
    const httpHandler = {
      handle: () => throwError({ httpStatusCode: 410, message: 'Bad Request' }),
    };
